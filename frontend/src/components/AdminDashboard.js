...
function AdminDashboard() {
  ...
  const updateOrderInState = (updatedOrder) => {
    ...
    setOrders(prevOrders => {
      ...
      if (updatedOrder.status === 'pending' || updatedOrder.status === 'out_for_delivery') {
        if (exists) {
  ...
  const fetchAllOrders = async () => {
    try {
      ...
      const filteredOrders = response.data.filter(
        order => order.status === 'pending' || order.status === 'out_for_delivery'
      );
  ...
  const assignDeliveryPerson = async (orderId, deliveryPersonId) => {
    try {
      ...
      const selectedDeliveryPerson = deliveryPersons.find(person => person.id === deliveryPersonId);
  ...
  <div className="max-w-7xl mx-auto">
    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
      <h3 className="text-xl sm:text-2xl font-bold text-white">Menu Management</h3>
      <button 
    ...
    {menuItems.length === 0 ? (
      ...
      <div className="bg-gray-800 rounded-xl shadow-lg overflow-hidden">
        ...
        <div className="block sm:hidden">
          {menuItems.map(item => (
            <div key={item.id} className="border-b border-gray-700 last:border-b-0 p-4">
        ...
        <div className="hidden sm:block overflow-x-auto">
          <table className="min-w-full">
            <thead className="bg-gray-700">
              <tr>
                <th className="py-4 px-6 text-left text-sm font-semibold text-gray-200 uppercase tracking-wider">Name</th>
                <th className="py-4 px-6 text-left text-sm font-semibold text-gray-200 uppercase tracking-wider">Category</th>
                <th className="py-4 px-6 text-left text-sm font-semibold text-gray-200 uppercase tracking-wider">Price</th>
                <th className="py-4 px-6 text-left text-sm font-semibold text-gray-200 uppercase tracking-wider">Status</th>
                <th className="py-4 px-6 text-left text-sm font-semibold text-gray-200 uppercase tracking-wider">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-700">
              {menuItems.map(item => (
                <tr key={item.id} className="hover:bg-gray-750 transition-colors">
                  ...
                  <td className="py-4 px-6 text-orange-500 font-bold">${item.price}</td>
  ...
  const renderOrdersManagement = () => {
    return (
      <div className="bg-gray-900 text-white min-h-screen p-4 sm:p-6 lg:p-8">
  <div className="max-w-7xl mx-auto">
    <h3 className="text-xl sm:text-2xl font-bold text-white mb-6">Active Orders</h3>
    ...
    {orders.length === 0 ? (
      ...
      <div className="bg-gray-800 rounded-xl shadow-lg overflow-hidden">
        ...
        <div className="block lg:hidden">
          {orders.map(order => (
            <div key={order.id} className="border-b border-gray-700 last:border-b-0 p-4">
              <div className="flex justify-between items-start mb-3">
                <div className="flex-1">
                  <div className="flex items-center space-x-2 mb-2">
                    <h4 className="font-semibold text-white text-lg">#{order.token}</h4>
                    <span className={`inline-block px-3 py-1 rounded-full text-xs font-medium ${
                      order.status === 'pending' ? 'bg-yellow-500 bg-opacity-20 text-yellow-400 border border-yellow-500' : 
                      order.status === 'out_for_delivery' ? 'bg-orange-500 bg-opacity-20 text-orange-400 border border-orange-500' : 
                  ...
                  <p className="text-gray-400 text-sm mb-1">
                    <span className="font-medium">Customer:</span> {order.user_name || 'Unknown'}
                  </p>
                  <p className="text-gray-400 text-sm mb-1">
                    <span className="font-medium">Delivery:</span> {
                      order.delivery_person ? (
                        <span className="text-green-400">{order.delivery_person.name}</span>
                      ) : (
                        <span className="text-red-400">Not assigned</span>
                      )
                    }
                  </p>
                  <p className="text-orange-500 font-bold text-lg">${order.total_price}</p>
        ...
        <div className="hidden lg:block overflow-x-auto">
          <table className="min-w-full">
            <thead className="bg-gray-700">
              <tr>
                <th className="py-4 px-6 text-left text-sm font-semibold text-gray-200 uppercase tracking-wider">Order #</th>
                <th className="py-4 px-6 text-left text-sm font-semibold text-gray-200 uppercase tracking-wider">Customer</th>
                <th className="py-4 px-6 text-left text-sm font-semibold text-gray-200 uppercase tracking-wider">Status</th>
                <th className="py-4 px-6 text-left text-sm font-semibold text-gray-200 uppercase tracking-wider">Delivery Person</th>
                <th className="py-4 px-6 text-left text-sm font-semibold text-gray-200 uppercase tracking-wider">Total</th>
                <th className="py-4 px-6 text-left text-sm font-semibold text-gray-200 uppercase tracking-wider">Date</th>
                <th className="py-4 px-6 text-left text-sm font-semibold text-gray-200 uppercase tracking-wider">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-700">
              {orders.map(order => (
                <tr key={order.id} className="hover:bg-gray-750 transition-colors">
                  <td className="py-4 px-6 text-white font-mono font-medium">#{order.token}</td>
                  <td className="py-4 px-6 text-gray-300">{order.user_name || 'Unknown'}</td>
                  <td className="py-4 px-6">
                    <span className={`inline-block px-3 py-1 rounded-full text-xs font-medium ${
                      order.status === 'pending' ? 'bg-yellow-500 bg-opacity-20 text-yellow-400 border border-yellow-500' : 
                      order.status === 'out_for_delivery' ? 'bg-orange-500 bg-opacity-20 text-orange-400 border border-orange-500' : 
                  ...
                  <td className="py-4 px-6">
                    {order.delivery_person ? (
                      <span className="font-medium text-green-400">{order.delivery_person.name}</span>
                    ) : (
                      <span className="text-red-400">Not assigned</span>
                    )}
                  </td>
                  <td className="py-4 px-6 text-orange-500 font-bold">${order.total_price}</td>
                  <td className="py-4 px-6 text-gray-400 text-sm">{new Date(order.created_at).toLocaleString()}</td>
                  <td className="py-4 px-6">
                    <div className="flex gap-2">
                      {order.status === 'pending' && (
                        <>
    ...
    {showDeliveryModal && (
      <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
        <div className="bg-gray-800 rounded-xl shadow-2xl p-6 max-w-md w-full border border-gray-700">
          <h3 className="text-xl font-bold text-white mb-6">Assign Delivery Person</h3>
          ...
          {deliveryPersons.length === 0 ? (
            ...
            <div className="mb-6">
              <p className="text-gray-300 mb-4">Select a delivery person:</p>
              <div className="space-y-3 max-h-60 overflow-y-auto">
                {deliveryPersons.map(person => (
                  <div 
                    ...
                    onClick={() => assignDeliveryPerson(selectedOrderId, person.id)}
                  >
  ...
  <div className="max-w-7xl mx-auto">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
      ...
      <div className="bg-gray-800 p-6 rounded-xl shadow-lg border border-gray-700">
        <div className="flex items-center justify-between mb-6">
          <h4 className="text-lg font-semibold text-white">Order Trends</h4>
          <div className="bg-blue-500 bg-opacity-20 px-3 py-1 rounded-full">
            <span className="text-blue-400 text-sm font-medium">Orders</span>
          </div>
        </div>
        <div className="h-64 bg-gray-750 rounded-lg p-4">
          <Line
            data={{
              ...
              datasets: [{
                label: 'Total Orders',
                data: analyticsData.orderTrends.map(item => item.count),
                borderColor: '#3B82F6',
      ...
      <div className="bg-gray-800 p-6 rounded-xl shadow-lg border border-gray-700">
        <div className="flex items-center justify-between mb-6">
          <h4 className="text-lg font-semibold text-white">Revenue Trends</h4>
        ...
        <div className="h-64 bg-gray-750 rounded-lg p-4">
          <Bar
            data={{
              ...
              datasets: [{
                label: 'Revenue ($)',
                data: analyticsData.revenueTrends.map(item => item.total),
    ...
    <div className="bg-gray-800 p-6 rounded-xl shadow-lg border border-gray-700">
      <div className="flex items-center justify-between mb-6">
        <h4 className="text-lg font-semibold text-white">Most Popular Items</h4>
        <div className="bg-orange-500 bg-opacity-20 px-3 py-1 rounded-full">
          <span className="text-orange-400 text-sm font-medium">Popular</span>
        </div>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="h-80 bg-gray-750 rounded-lg p-4">
            <Pie
              data={{
                ...
                datasets: [{
                  data: analyticsData.popularItems.map(item => item.count),
                  backgroundColor: [
        ...
        <div className="lg:col-span-1">
          <div className="bg-gray-750 rounded-lg p-4 h-80 overflow-y-auto hide-scrollbar">
            <h5 className="text-sm font-semibold text-gray-300 mb-4 uppercase tracking-wider">Items</h5>
  ...
  return (
    <div className="max-w-7xl mx-auto p-4">
      ...
      <p className="mb-6">Your role: <span className="inline-block px-2 py-1 bg-red-100 text-red-800 rounded">{currentUser.user.role}</span></p>
      ...
      <div className="flex border-b mb-6">
        ...
        <button 
          className={`py-2 px-4 font-medium ${activeTab === 'menu' ? 'border-b-2 border-blue-500 text-blue-500' : 'text-gray-500'}`}
          onClick={() => setActiveTab('menu')}
        >
          Menu
        </button>
        <button 
          className={`py-2 px-4 font-medium ${activeTab === 'analytics' ? 'border-b-2 border-blue-500 text-blue-500' : 'text-gray-500'}`}

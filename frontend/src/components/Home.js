...
function Home() {
    ...
    const fetchMenu = async () => {
      try {
        ...
        const specials = items
          .filter((item) => item.isSpecial || Math.random() > 0.7)
          .slice(0, 5);
  ...
  const prevSlide = () => {
    setCurrentSlide(
      (prev) => (prev - 1 + todaysSpecials.length) % todaysSpecials.length
    );
  };
                        ...
                        <div className="flex flex-col sm:flex-row">
                          ...
                          <div className="sm:w-1/2 p-4 sm:p-6 flex flex-col justify-center">
                            <div className="flex items-center mb-2">
                              <span className="bg-orange-500 text-xs px-2 py-1 rounded-full font-medium mr-2">
                                SPECIAL
                              </span>
                              {item.rating && (
